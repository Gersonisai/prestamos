...
export function exportExcel(loans: Loan[]) {
  ...
  const payments:any[] = []
  loans.forEach(l => (l.payments||[]).forEach(p => payments.push({
    loanId: l.id, Cliente: l.clientName, FechaPago: p.date, Tipo: p.type, Metodo: p.method, Monto: p.amount
  })))
